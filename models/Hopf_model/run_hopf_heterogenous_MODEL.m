...
function [ts_simulated_all,simulation_params] = run_hopf_heterogenous_MODEL(empirical_params,simulation_params)
	...
	% Now will have to calculate what the bifurcation parameters are for every set
	for g_ind=1:length(G),		
		disp(['Optimzing: bifurcation parameters for G=',num2str(G(g_ind))]);
		[~,bifpar,~,sc] =  hopf_optimization_bifurcation(C,G(g_ind),f_peak,empirical_params.time_series(:,:,:,simulation_params.preproMethod),empirical_params.TR,total_time,simulation_params.UseGSR);		
		simulation_params.stopping_criteria{g_ind} = sc;
		simulation_params.bifpar(:,g_ind) = bifpar;
	end
